...
class BitcoinEclipseModel(BlockchainModel):
    def __init__(self, alpha: float, WW: float, gamma: float, max_fork: int):
        ...
        self.Action = self.create_int_enum('Action',
                                           ['Illegal', 'Adopt', 'Override', 'Match', 'Wait', 'Switch', 'Reveal'])
    ...
    def get_state_space(self) -> Space:
        state_types = [(0, self.max_fork), (0, self.max_fork), (0, self.max_fork), (0, self.max_fork)] + [
            self.BlockStatus, self.Miner] * self.max_fork + [self.BlockStatus, self.Miner] * self.max_fork + [
                          self.Fork] + [self.Fork]
    ...
    def get_initial_state(self) -> BlockchainModel.State:
        return self.combine_state(0, 0, 0, 0, self.initializeVector(), self.initializeVector(), self.Fork.Irrelevant,
                                  self.Fork.Irrelevant)

    def get_final_state(self) -> BlockchainModel.State:
        return self.combine_state(-1, -1, -1, -1, self.final_v(), self.final_v(), self.Fork.Irrelevant,
                                  self.Fork.Irrelevant)
        ...
        if action_type is self.Action.Adopt and group is self.Group.Green:
            ...
            if att_down > blue and (self.max_fork - taken) >= (blue + 1):
                next_state = self.combine_state(att_up, green, att_down - blue - 1, 0, v_ag,
                                                ...
                                                fork_green, self.Fork.Irrelevant)
                transitions.add(next_state, probability=1)
            else:
                transitions.add(self.final_state, probability=1, reward=self.error_penalty)
            ...
            if fork_green is not self.Fork.Active and fork_blue is not self.Fork.Active:
                if att_up < self.max_fork and green < self.max_fork and blue < self.max_fork:
                    ...
                    next_state = self.combine_state(att_up, green, att_down, blue + 1, v_ag, v_ab, fork_green,
                                                    self.Fork.Relevant)
                ...
                if att_up < self.max_fork and green < self.max_fork and blue < self.max_fork:
                    ...
                    if att_up >= green and vacant > green:
                        ...
                        next_state = self.combine_state(att_up - green, 1, att_down, blue,
                                                        self.set_block_status(v_ag, green, self.Miner.Attacker, taken),
                                                        v_ab, self.Fork.Relevant,
                                                        fork_blue)
                        transitions.add(next_state, probability=(self.gamma * self.WW))

                        next_state = self.combine_state(att_up, green + 1, att_down, blue, v_ag, v_ab,
                                                        self.Fork.Relevant,
                                                        fork_blue)
                ...
                if att_up < self.max_fork and green < self.max_fork and blue < self.max_fork:
                    ...
                    if att_down >= blue and vacant > blue:
                        ...
                        next_state = self.combine_state(att_up, green, att_down - blue, 1, v_ag,
                                                        self.set_block_status(v_ab, blue, self.Miner.Attacker, taken),
                ...
                if att_up < self.max_fork and green < self.max_fork and blue < self.max_fork:
                    ...
                    if att_down > blue and vacant_b > (blue + 1) and att_up > green and vacant_g > (green + 1):
                        ...
                        next_state = self.combine_state(att_up - green, 1, att_down, blue,
                                                        self.set_block_status(v_ag, green, self.Miner.Attacker,
                                                                              taken_g), v_ab,
                        ...
                        next_state = self.combine_state(att_up, green, att_down - blue, 1, v_ag,
                                                        self.set_block_status(v_ab, blue, self.Miner.Attacker, taken_b),
                                                        fork_green, self.Fork.Relevant)
                        transitions.add(next_state, probability=(self.gamma * (1 - self.WW - self.alpha)))
